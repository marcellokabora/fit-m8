<script lang="ts">
	...
	onMount(() => {
		...
		unsubscribe = onSnapshot(q, (snap) => {
			matches = snap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<Match, 'id'>) }));
			loading = false;
		});
	});
...
<div class="flex min-h-screen flex-col bg-bg pb-24">
	...
	{:else if matches.length === 0}
		<div class="flex flex-1 flex-col items-center justify-center gap-4 px-8 text-center">
			...
			<p class="text-sm text-muted">Keep swiping to find your sports partner!</p>
			<a
				href="/discover"
	...
	{:else}
		<div class="flex flex-col gap-3 px-5">
			{#each matches as match}
				...
				>
					<div class="flex size-14 shrink-0 items-center justify-center rounded-full bg-primary/10 text-3xl">
						{activity?.emoji ?? '🏃'}
					</div>
					<div class="flex-1 min-w-0">
						<p class="font-bold text-text truncate">{activity?.label ?? match.activity}</p>
						<p class="text-sm text-muted">{match.format} · {match.status}</p>
					</div>
					<div class="flex flex-col items-end gap-1">
						<span class="rounded-full bg-primary px-2.5 py-0.5 text-xs font-bold text-white">Chat</span>
					</div>
				</a>
			{/each}
		</div>
