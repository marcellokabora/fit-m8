<script lang="ts">
	...
	onMount(() => {
		...
		unsubscribe = onSnapshot(q, async (snap) => {
			messages = snap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<Message, 'id'>) }));
			loading = false;
			await tick();
...
<div class="flex h-screen flex-col bg-bg">
	<!-- Header -->
	<div class="flex items-center gap-3 border-b border-gray-200 bg-surface px-4 pb-3 pt-12">
		<a href="/matches" class="flex size-9 items-center justify-center rounded-full hover:bg-gray-100">
			←
		</a>
		<div class="flex-1">
	...
	<div class="flex-1 overflow-y-auto px-4 py-4">
		...
		{:else}
			{#each messages as msg}
				<div
					class="mb-3 flex {msg.senderId === currentUid ? 'justify-end' : 'justify-start'}"
				>
					<div
						class="max-w-[75%] rounded-2xl px-4 py-3 text-sm {msg.senderId === currentUid
