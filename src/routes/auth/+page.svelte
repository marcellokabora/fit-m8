<script lang="ts">
	...
	let mode = $derived(page.url.searchParams.get('mode') === 'login' ? 'login' : 'register');
	let email = $state('');
	let password = $state('');
	let error = $state('');
	...
	async function handleSocial(provider: 'google' | 'apple' | 'facebook') {
		...
		try {
			...
			else if (provider === 'apple') await authUser.signInApple();
...
<div class="flex min-h-screen flex-col bg-bg px-6 pb-10 pt-16">
	...
	<div class="mb-8 flex flex-col items-center gap-2">
		<a href="/" class="mb-2 flex size-14 items-center justify-center rounded-2xl bg-primary">
		...
		<p class="text-sm text-muted">
			{mode === 'login' ? 'Sign in to find your next match' : 'Create your sports profile'}
		</p>
	</div>
	...
	<div class="flex flex-col gap-3">
		...
		<button
			...
			disabled={loading}
			class="flex w-full items-center justify-center gap-3 rounded-2xl bg-text py-4 text-base font-semibold text-white shadow-sm active:scale-95 disabled:opacity-50"
		>
